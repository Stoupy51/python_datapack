...
from ..utils.io import write_to_versioned_function, write_to_file, write_to_load_file
...
def main(config: dict):
	...
	write_to_versioned_function(config, "load/enumerate", f"""
...
""")
	write_to_versioned_function(config, "load/resolve", f"""
...
""")
	...
	write_to_file(f"{config['build_datapack']}/data/{namespace}/tags/function/enumerate.json", stp.super_json_dump({"values": [f"{namespace}:v{version}/load/enumerate"]}))
	write_to_file(f"{config['build_datapack']}/data/{namespace}/tags/function/resolve.json", stp.super_json_dump({"values": [f"{namespace}:v{version}/load/resolve"]}))

	# Setup load main function
	write_to_versioned_function(config, "load/main", f"""
